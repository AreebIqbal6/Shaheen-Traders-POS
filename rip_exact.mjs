...
const newDesktop = `              <div className="overflow-y-auto custom-scrollbar h-full w-full">
...
const newMobile = `              <div className="overflow-y-auto custom-scrollbar h-full w-full flex flex-col divide-y divide-slate-200 dark:divide-slate-700">
                {filteredProducts.map((product, index) => {
                  const mStock = minStockDict[product.id] ?? 5;
                  const isCrit = product.stock <= mStock;
                  const isWarn = product.stock > mStock && product.stock <= mStock + 5;
...
// Replace 672-709 first (bottom to top so indices don't shift)
lines.splice(672, 709 - 672 + 1, ...newMobile);

// Replace 622-669
lines.splice(622, 669 - 622 + 1, ...newDesktop);
